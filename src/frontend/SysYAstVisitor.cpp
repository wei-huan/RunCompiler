...
antlrcpp::Any SysYAstVisitor::visitConstDecl(
    SysYParser::ConstDeclContext *ctx) {
...
void SysYAstVisitor::dfs_const_init(SysYParser::ListConstInitValContext *node,
    ...
    if (shape.size() == 0) throw RuntimeError("Invalid Init List");
    ...
    for (auto child : node->constInitVal()) {
        if (auto scalar_child =
            ...
            if (cnt + 1 > total_size) throw RuntimeError("Invalid Init List");
            result.emplace_back(
                scalar_child->constExp()->accept(this).as<int32_t>());
            ++cnt;
        } else {
            auto list_child =
...
vector<int32_t> SysYAstVisitor::parse_const_init(
    SysYParser::ConstInitValContext *root, const vector<int32_t> &shape) {
    vector<int32_t> ret;
    if (auto scalar_root =
            dynamic_cast<SysYParser::ScalarConstInitValContext *>(root)) {
        if (shape.size()) throw RuntimeError("Invalid Init List");
...
antlrcpp::Any SysYAstVisitor::visitConstDef(SysYParser::ConstDefContext *ctx) {
    ...
    SSALeftValue lvalue(left_id, cur_type, ident_name, shape, ssa_init_values, true);
...
antlrcpp::Any SysYAstVisitor::visitUninitVarDef(
...
void SysYAstVisitor::dfs_var_init(SysYParser::ListInitvalContext *node,
    ...
    for (auto child : node->initVal()) {
        if (auto scalar_child =
            ...
            if (cnt + 1 > total_size) throw RuntimeError("Invalid Init List");
            if (value_mode == Const) {
                auto ssa_res = SSARightValue(cur_type, scalar_child->exp()->accept(this));
            ...
            auto list_child =
                dynamic_cast<SysYParser::ListInitvalContext *>(child);
            assert(list_child);
            if (cnt % child_size != 0 || cnt + child_size > total_size)
                throw RuntimeError("Invalid Init List");
...
vector<SSARightValue> SysYAstVisitor::parse_var_init(
...
antlrcpp::Any SysYAstVisitor::visitInitVarDef(
    ...
    // 在函数体里就创建 Alloca IR
    if (cur_func_name != "_init") {
        cur_bb->push_ir_instr(new AllocaIR(lvalue));  // Alloca IR don't need init value information
    }
...
antlrcpp::Any SysYAstVisitor::visitFuncFParams(
...
antlrcpp::Any SysYAstVisitor::visitFuncFParam(
    SysYParser::FuncFParamContext *ctx) {
...
antlrcpp::Any SysYAstVisitor::visitBlockItem(
    SysYParser::BlockItemContext *ctx) {
...
antlrcpp::Any SysYAstVisitor::visitAssignment(
    SysYParser::AssignmentContext *ctx) {
...
antlrcpp::Any SysYAstVisitor::visitBlockStmt(
    SysYParser::BlockStmtContext *ctx) {
...
antlrcpp::Any SysYAstVisitor::visitWhileStmt(
    SysYParser::WhileStmtContext *ctx) {
...
antlrcpp::Any SysYAstVisitor::visitBreakStmt(
...
antlrcpp::Any SysYAstVisitor::visitContinueStmt(
...
antlrcpp::Any SysYAstVisitor::visitPrimaryExp1(
...
antlrcpp::Any SysYAstVisitor::visitPrimaryExp2(
...
antlrcpp::Any SysYAstVisitor::visitPrimaryExp3(
    ...
    if (value_mode == ValueMode::Const) {
        ...
        if (opcode == '+') {
            ...
            if (ssa.id) {
                ...
                cur_bb->push_ir_instr(new IcmpIR(ret1_ssa, ssa, temp1_ssa, IcmpType::NEQ));
...
antlrcpp::Any SysYAstVisitor::visitFuncRParams(
...
antlrcpp::Any SysYAstVisitor::visitExpAsRParam(
